...
export function useWorkRecords(): UseWorkRecordsResult {
  ...
  const add = useCallback(async (record: Omit<WorkRecord, 'id' | 'createdAt'>) => {
    const result = await WorkLedgerService.add(record);
    refetch();
    return result;
  }, [refetch]);

  const update = useCallback(async (id: string, patch: Partial<WorkRecord>) => {
    const result = await WorkLedgerService.update(id, patch);
    refetch();
    return result;
  }, [refetch]);

  const verify = useCallback(async (id: string, verifierName: string) => {
