...
export default function NotificationsPage() {
  ...
  const handleWorkflowAction = async (
    ...
    setFeedback("");
    try {
      const response = await InboxService.actOnItem(
        notification.id,
        action.payload,
      );
      await refresh();
      setFeedback(
        `${notification.title} marked as ${response?.result || action.key}.`,
      );
    } catch (error) {
      console.error("[NotificationsPage] Failed workflow action", error);
      setFeedback(
        `Could not complete "${action.label}" for ${notification.title}.`,
      );
    } finally {
      setBusyItemId(null);
    }
    ...
    <div className="space-y-6 max-w-[1120px] mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <GlassCard className="p-4">
          ...
          <p className="mt-2 text-3xl font-bold text-primary/90">
            {standardInboxItems.length}
          </p>
          <p className="text-xs text-muted-foreground mt-1">
          ...
          <div className="space-y-3">
            {documentChangeAlerts.map((alert) => (
              <DocumentChangeReviewCard
                ...
                onOpenPl={() =>
                  navigate(
                    `/pl/${alert.plId}?tab=crossrefs&doc=${alert.documentId}`,
                  )
                }
                onApprove={async () => {
                  ...
                  try {
                    await approveAlert(
                      alert.id,
                      "Approved from notifications inbox",
                    );
                    setFeedback(
                      `Approved latest document update for PL ${alert.plNumber}.`,
                    );
                  } catch (error) {
                    console.error(
                      "[NotificationsPage] Failed to approve supervisor review",
                      error,
                    );
                    setFeedback(
                      `Could not approve the document update for PL ${alert.plNumber}.`,
                    );
                  } finally {
                    setBusyItemId(null);
                  }
                ...
                onBypass={async () => {
                  ...
                  try {
                    ...
                    setFeedback(
                      `Bypassed latest document update for PL ${alert.plNumber}.`,
                    );
                  } catch (error) {
                    console.error(
                      "[NotificationsPage] Failed to bypass supervisor review",
                      error,
                    );
                    setFeedback(
                      `Could not bypass the document update for PL ${alert.plNumber}.`,
                    );
                  } finally {
                    setBusyItemId(null);
                  }
      ...
      <GlassCard className="p-5">
        <div className="mb-4">
          ...
          <p className="text-sm text-muted-foreground">
            Open the affected page directly from each alert, or act from here
          ...
          {standardItemsWithActions.map((notification) =>
            (() => {
              const previewDocumentId =
                resolveNotificationPreviewDocumentId(notification);
              return (
                <div
                  key={notification.id}
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-sm text-foreground/90 mt-1">
                        {notification.subtitle ||
                          "Action required in the EDMS workflow queue."}
                      </p>
