...
import { Shield, Loader2, AlertCircle, Eye, EyeOff, Lock, Clock, ChevronDown, ChevronUp } from 'lucide-react';
...
export function Structured() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setIsLoading(false);
      if (username !== 'admin' && username !== 'a.kowalski' && username !== 'm.chen' && username !== 's.patel') {
            ...
            <div className="w-full p-4 bg-slate-900/30 border border-slate-800/50 rounded-xl mb-4">
              <div className="space-y-1">
                {[
                  ...
                  >
                    <div>
                      <span className="font-mono text-teal-400/80 group-hover:text-teal-400">{c.u}</span> 
                      <span className="mx-1 text-slate-600">/</span> 
                      <span className="font-mono text-slate-500">{c.p}</span>
                    </div>
                    <span className="text-slate-600 font-medium text-[10px] uppercase tracking-wider">{c.r}</span>
                  </button>
                ))}
              </div>
            </div>
