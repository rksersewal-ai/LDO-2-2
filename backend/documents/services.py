...
def _enqueue_or_inline(
    task_callable, task_arg, inline_callable, *, operation_name: str
):
    try:
        ...
        logger.warning(
            "%s task enqueue failed for %s: %s", operation_name, task_arg, exc
        )
        inline_callable()
        return None, "inline"
...
class DocumentService:
    ...
    def queryset(params, user=None):
        queryset = PermissionService.scope_queryset(
            Document.objects.all(), user, "view_document"
        )
        ...
        if hash_status == "present":
            queryset = queryset.exclude(fingerprint_3x64k__isnull=True).exclude(
                fingerprint_3x64k=""
            )
            ...
            queryset = queryset.filter(
                Q(fingerprint_3x64k__isnull=True) | Q(fingerprint_3x64k="")
            )
    ...
    def ingest(validated_data, user, request):
        ...
        revision_label = (
            validated_data.get("revision_label") or ""
        ).strip() or f"{validated_data['normalized_revision']}"
        ...
        if (
            ...
            and validated_data["category"] not in metadata_tags
        ):
        ...
        index_job_id, index_job_mode = _enqueue_or_inline(
            ...
            document.id,
            inline_callable=lambda: DocumentIndexOrchestrator.index_document(
                document, force_hashes=True
            ),
        ...
        if validated_data.get("ocr_requested", True):
            ocr_job, _ = OcrApplicationService.start_job(
                str(document.id), user, request
            )
...
class DocumentMetadataService:
    ...
    def list_entities(cls, document_id, user=None):
        ...
        return DocumentOcrEntity.objects.filter(document=document).order_by(
    ...
    def list_assertions(cls, document_id, user=None):
        document = cls.get_document(document_id, user)
        return DocumentMetadataAssertion.objects.filter(document=document).order_by(
            "field_key", "-updated_at"
        )

    @staticmethod
    def _set_entity_review(
        entity: DocumentOcrEntity, *, status_value: str, user=None, notes: str = ""
    ):
    ...
    @classmethod
    def promote_entity_to_assertion(
        cls, document_id, entity_id, field_key: str, user=None, notes: str = ""
    ):
        document = cls.get_document(document_id, user)
        entity = DocumentOcrEntity.objects.get(document=document, id=entity_id)
        cls._set_entity_review(entity, status_value="CONFIRMED", user=user, notes=notes)
    ...
    def create_assertion(
        ...
        *,
        ...
        document = cls.get_document(document_id, user)
        normalized_value = DocumentOcrProcessingService._normalize_entity_value(
            field_key.upper(), value
        )
        assertion = DocumentMetadataAssertion.objects.create(
            document=document,
            field_key=field_key,
            ...
            status=status_value,
            approved_by=user
            if status_value == "APPROVED" and getattr(user, "is_authenticated", False)
            else None,
    ...
    def approve_assertion(cls, document_id, assertion_id, user=None, notes: str = ""):
        document = cls.get_document(document_id, user)
        assertion = DocumentMetadataAssertion.objects.get(
            document=document, id=assertion_id
        )
        assertion.status = "APPROVED"
        assertion.approved_by = (
...
class IndexedSourceService:
    ...
    def create_or_update(validated_data, request=None):
        ...
        if source_id:
            source, _ = IndexedSource.objects.update_or_create(
                id=source_id, defaults=defaults
            )
        else:
    ...
    def disable_source_schedule(cls, source_or_id):
        ...
        PeriodicTask.objects.filter(name=cls._source_schedule_name(source_id)).update(
            enabled=False
        )
    ...
    def ensure_hash_backfill_schedule(cls):
        interval = max(
            int(getattr(settings, "EDMS_HASH_BACKFILL_INTERVAL_MINUTES", 0) or 0), 0
        )
        if interval <= 0:
            PeriodicTask.objects.filter(name=cls.HASH_BACKFILL_SCHEDULE_NAME).update(
                enabled=False
            )
            return
        ...
        PeriodicTask.objects.update_or_create(
            ...
            defaults={
                ...
                "kwargs": json.dumps(
                    {
                        "batch_size": int(
                            getattr(settings, "EDMS_HASH_BACKFILL_BATCH_SIZE", 500)
                            or 500
                        ),
                        "force_full_hash": bool(
                            getattr(
                                settings, "EDMS_HASH_BACKFILL_FORCE_FULL_HASH", False
                            )
                        ),
                    }
                ),
    ...
    def sync_runtime_schedules(cls):
        sources = list(
            IndexedSource.objects.all().only("id", "is_active", "scan_interval_minutes")
        )

        inactive_names = [
            cls._source_schedule_name(s.id) for s in sources if not s.is_active
        ]
        if inactive_names:
        ...
        if active_sources:
            intervals = {
                max(int(s.scan_interval_minutes or 60), 1) for s in active_sources
            }
            schedule_map = {}
            ...
            existing_tasks = {
                t.name: t for t in PeriodicTask.objects.filter(name__in=active_names)
            }
            ...
            if tasks_to_update:
                PeriodicTask.objects.bulk_update(
                    tasks_to_update, ["task", "interval_id", "args", "enabled"]
                )
...
class _DocumentIndexingBatchService:
    ...
    def _allowed_extension(path: Path, source: IndexedSource) -> bool:
        ...
        if source.include_extensions and extension not in {
            item.lower() for item in source.include_extensions
        }:
            return False
    ...
    def _generate_document_id(path: Path) -> str:
        stable_hash = (
            hashlib.sha1(str(path).lower().encode("utf-8")).hexdigest()[:12].upper()
        )
        return f"NET-{stable_hash}"

    @classmethod
    def index_path(
        cls, path: Path, source: IndexedSource, *, force_full_hash: bool = False
    ):
    ...
    @classmethod
    def index_paths_bulk(
        cls, paths: list[Path], source: IndexedSource, *, force_full_hash: bool = False
    ) -> list[Document]:
...
class IndexedSourceFileStateService:
    ...
    def state_for_relative_path(cls, source: IndexedSource, relative_path: str):
        return (
            IndexedSourceFileState.objects.filter(
                source=source, relative_path=relative_path
            )
    ...
    @classmethod
    def _update_document_source_metadata(
        cls,
        ...
        source_index.update(
            {
                ...
                if last_indexed_at
                else source_index.get("last_indexed_at", ""),
            }
        )
        search_metadata["source_index"] = source_index
        Document.objects.filter(pk=document.pk).update(
            search_metadata=search_metadata, updated_at=timezone.now()
        )
    ...
    def mark_success(cls, source: IndexedSource, path: Path, document: Document):
        ...
        cls._update_document_source_metadata(
            document, source, relative_path, source_state="ACTIVE", last_indexed_at=now
        )
        return state

    @classmethod
    def mark_success_bulk(
        cls, source: IndexedSource, paths: list[Path], documents: list[Document]
    ):
        now = timezone.now()
        ...
        for path in paths:
            ...
            states_to_create_or_update.append(
                IndexedSourceFileState(
                    source=source,
                    relative_path=relative_path,
                    absolute_path=str(path),
            ...
            cls._update_document_source_metadata(
                doc, source, relative_path, source_state="ACTIVE", last_indexed_at=now
            )
    ...
    @classmethod
    def mark_missing_for_unseen_paths(
        cls, source: IndexedSource, seen_relative_paths: set[str]
    ):
        now = timezone.now()
        queryset = IndexedSourceFileState.objects.filter(source=source).exclude(
            relative_path__in=seen_relative_paths
        )
        updated_states = []
        for state in queryset.select_related("document"):
            ...
            state.save(
                update_fields=["status", "missing_since", "last_error", "updated_at"]
            )
            cls._update_document_source_metadata(
                state.document,
                source,
    ...
    def relink_moved_path(cls, source: IndexedSource, old_path: str, new_path: str):
        ...
        if (
            expected_document.fingerprint_3x64k
            and expected_document.fingerprint_3x64k != sparse_hash
        ):
            return None
        ...
        indexed_document = DocumentIndexOrchestrator.index_document(
            expected_document, force_hashes=True, force_full_hash=False
        )
        cls._update_document_source_metadata(
            indexed_document,
            source,
...
class CrawlJobService:
    ...
    def run_job(job: CrawlJob, *, force_full_hash: bool = False):
        ...
        skip_indexed = bool(parameters.get("skip_indexed"))
        ...
        try:
            if old_path and new_path:
                moved_document = IndexedSourceFileStateService.relink_moved_path(
                ...
                if moved_document is not None:
                    ...
                    seen_relative_paths.add(
                        IndexedSourceFileStateService._relative_path(
            ...
            indexed_state_by_relative: dict[str, IndexedSourceFileState] = {}
            if skip_indexed:
                state_queryset = IndexedSourceFileState.objects.filter(
                    source=source
                ).select_related("document")
                if scan_paths:
                    relative_targets = {
                    ...
                    if relative_targets:
                        state_queryset = state_queryset.filter(
                            relative_path__in=relative_targets
                        )
                    else:
                        state_queryset = state_queryset.none()
                indexed_state_by_relative = {
                    state.relative_path: state for state in state_queryset
                }
            ...
            for path in candidate_paths:
                ...
                if skip_indexed:
                    relative_path = IndexedSourceFileStateService._relative_path(
                        source, path
                    )
                    existing_state = indexed_state_by_relative.get(relative_path)
                    if (
                        existing_state
                        and existing_state.status == "ACTIVE"
                        and existing_state.document_id
                    ):
                        continue
            ...
            for i in range(0, len(valid_paths), batch_size):
                ...
                try:
                    indexed_documents = _DocumentIndexingBatchService.index_paths_bulk(
                        batch_paths, source, force_full_hash=force_full_hash
                    )

                    IndexedSourceFileStateService.mark_success_bulk(
                        source, batch_paths, indexed_documents
                    )

                    for path in batch_paths:
                        discovered += 1
                        seen_relative_paths.add(
                            IndexedSourceFileStateService._relative_path(source, path)
                        )
                    ...
                    for path in batch_paths:
                        ...
                        try:
                            document = _DocumentIndexingBatchService.index_path(
                                path, source, force_full_hash=force_full_hash
                            )
                            if document:
                                indexed += 1
                                IndexedSourceFileStateService.mark_success(
                            ...
                            IndexedSourceFileStateService.mark_failure(
                                source, path, str(exc)
                            )
            ...
            if scan_paths:
                ...
                for raw_path in scan_paths:
                    ...
                    if candidate.exists():
                        continue
                    missing_targets.append(
                        IndexedSourceFileStateService._relative_path(source, candidate)
                    )
                for relative_path in set(missing_targets):
                    IndexedSourceFileStateService.mark_path_missing(
                        source, relative_path
                    )
            else:
                IndexedSourceFileStateService.mark_missing_for_unseen_paths(
                    source, seen_relative_paths
                )
            ...
            source.last_successful_crawl_at = (
                timezone.now() if failed == 0 else source.last_successful_crawl_at
            )
...
class HashBackfillJobService:
    ...
    def run_job(job: HashBackfillJob, *, force_full_hash: bool = False):
        ...
        try:
            hash_gap = Q(fingerprint_3x64k__isnull=True) | Q(fingerprint_3x64k="")
            if force_full_hash:
                hash_gap |= Q(file_hash__isnull=True) | Q(file_hash="")

            queryset = Document.objects.filter(hash_gap).order_by("created_at")
            if job.source_id:
...
class InitialRunService:
    @staticmethod
    def pending_hash_queryset(*, force_full_hash: bool = False):
        queryset = Document.objects.all()
        hash_gap = Q(fingerprint_3x64k__isnull=True) | Q(fingerprint_3x64k="")
        if force_full_hash:
            hash_gap |= Q(file_hash__isnull=True) | Q(file_hash="")
        return queryset.filter(hash_gap).order_by("created_at")

    @staticmethod
    def pending_dedup_queryset():
        has_hash = (Q(fingerprint_3x64k__isnull=False) & ~Q(fingerprint_3x64k="")) | (
            Q(file_hash__isnull=False) & ~Q(file_hash="")
        )
        needs_group = Q(duplicate_group_key__isnull=True) | Q(duplicate_group_key="")
        stale_sparse_group = (
            Q(file_hash__isnull=False)
            & ~Q(file_hash="")
            & Q(duplicate_group_key__startswith="sparse:")
        )
        return (
            Document.objects.filter(has_hash)
            .filter(needs_group | stale_sparse_group)
            .order_by("created_at")
        )

    @staticmethod
    def pending_ocr_queryset():
        has_resolvable_file = (Q(file__isnull=False) & ~Q(file="")) | (
            Q(external_file_path__isnull=False) & ~Q(external_file_path="")
        )
        return (
            Document.objects.filter(
                Q(ocr_status="Not Started") | Q(ocr_status="Failed"),
            )
            .filter(has_resolvable_file)
            .order_by("created_at")
        )
    ...
    def summary(cls):
        duplicate_groups = (
            Document.objects.exclude(duplicate_group_key__isnull=True)
            .exclude(duplicate_group_key="")
            .values("duplicate_group_key")
            .annotate(group_size=Count("id"))
            .filter(group_size__gt=1)
        )
        active_sources = (
            IndexedSource.objects.filter(is_active=True)
            .annotate(
                tracked_files=Count("file_states"),
                active_files=Count(
                    "file_states", filter=Q(file_states__status="ACTIVE")
                ),
                missing_files=Count(
                    "file_states", filter=Q(file_states__status="MISSING")
                ),
                failed_files=Count(
                    "file_states", filter=Q(file_states__status="FAILED")
                ),
                indexed_documents=Count("file_states__document", distinct=True),
            )
            .order_by("name")
        )
        ...
        source_details = [
            {
                ...
                "tracked_files": source.tracked_files,
                "active_files": source.active_files,
                "missing_files": source.missing_files,
                "failed_files": source.failed_files,
                "indexed_documents": source.indexed_documents,
            }
            for source in active_sources
        ]
        ...
        return {
            "documents": {
                ...
                "indexed_documents": Document.objects.exclude(
                    search_indexed_at__isnull=True
                ).count(),
                "missing_search_index": Document.objects.filter(
                    search_indexed_at__isnull=True
                ).count(),
                "missing_sparse_hash": cls.pending_hash_queryset(
                    force_full_hash=False
                ).count(),
                "missing_full_hash": Document.objects.filter(
                    Q(file_hash__isnull=True) | Q(file_hash="")
                ).count(),
                "pending_ocr": cls.pending_ocr_queryset().count(),
                "processing_ocr": Document.objects.filter(
                    ocr_status="Processing"
                ).count(),
                "pending_dedup": cls.pending_dedup_queryset().count(),
                "duplicate_groups": duplicate_groups.count(),
                "duplicate_documents": Document.objects.filter(
                    duplicate_status="DUPLICATE"
                ).count(),
            },
            "sources": {
                "total_sources": IndexedSource.objects.count(),
                "active_sources": active_sources.count(),
                "tracked_files": IndexedSourceFileState.objects.filter(
                    source__is_active=True
                ).count(),
                "active_files": IndexedSourceFileState.objects.filter(
                    source__is_active=True, status="ACTIVE"
                ).count(),
                "missing_files": IndexedSourceFileState.objects.filter(
                    source__is_active=True, status="MISSING"
                ).count(),
                "failed_files": IndexedSourceFileState.objects.filter(
                    source__is_active=True, status="FAILED"
                ).count(),
            },
            "active_source_details": source_details,
            "latest_jobs": {
                "crawl": [
                    {
                        "id": str(job.id),
                    ...
                    for job in CrawlJob.objects.select_related("source").order_by(
                        "-created_at"
                    )[:5]
                ],
                "hash_backfill": [
                    {
                        ...
                        "full_hashes_computed": job.full_hashes_computed,
                        "error_message": job.error_message,
                    }
                    for job in HashBackfillJob.objects.select_related(
                        "source"
    ...
    @classmethod
    def run_dedup_backfill(
        cls, *, batch_size: int | None = None, force_full_hash: bool = False
    ):
        queryset = cls.pending_dedup_queryset()
        if batch_size:
    ...
    @classmethod
    def run_pending_ocr(cls, *, batch_size: int | None = None, user=None, request=None):
        queryset = cls.pending_ocr_queryset()
        if batch_size:
            queryset = queryset[:batch_size]
        ...
        for document_id in queryset.values_list("id", flat=True):
            try:
                OcrApplicationService.start_job(str(document_id), user, request=request)
                queued_documents += 1
            except Exception as exc:
...
class DuplicateDecisionService:
    ...
    def group_documents(params, user=None):
        queryset = (
            DocumentService.queryset(params, user)
            .filter(duplicate_group_key__isnull=False)
            .exclude(duplicate_group_key="")
        )
        ...
        for document in (
            queryset.select_related("duplicate_of")
            .prefetch_related("metadata_assertions", "ocr_entities")
            .order_by(
                "duplicate_group_key",
                "-source_modified_at",
                "-created_at",
        ...
        decisions = {
            ...
            for decision in DuplicateDecision.objects.filter(
                group_key__in=groups.keys()
            )
        }
        ...
        for group_key, documents in groups.items():
            ...
            master_document = next(
                (
                    document
                    for document in documents
                    if document.duplicate_status == "MASTER"
                ),
                documents[0],
            )
            ...
            payload.append(
                {
                    ...
                    "family_key": next(
                        (
                            ...
                            for document in documents
                            if document.document_family_key
                        ),
                        "",
                    ),
                    "decision_status": decision.status if decision else "PENDING",
                    "decision": decision.decision if decision else "MERGE",
                    "master_document_id": master_document.id
                    if master_document
                    else None,
                    "document_count": len(documents),
                    "total_bytes": total_bytes,
                    "potential_savings_bytes": max(
                        total_bytes - int(master_document.size or 0), 0
                    )
                    if master_document
                    else 0,
                    "documents": documents,
                    ...
                    "hash_status": {
                        "present": sum(
                            1
                            for document in documents
                            if document.file_hash or document.fingerprint_3x64k
                        ),
                        "full": sum(1 for document in documents if document.file_hash),
                        "missing": sum(
                            1
                            for document in documents
                            if not document.fingerprint_3x64k and not document.file_hash
                        ),
                    },
                    "risk_flags": {
    ...
    @transaction.atomic
    def apply_decision(
        group_key, *, master_document_id=None, decision="MERGE", notes="", user=None
    ):
        ...
        if master_document_id:
            try:
                master_document = next(
                    document
                    for document in documents
                    if document.id == master_document_id
                )
            except StopIteration as exc:
                raise ValidationError(
                    {"master_document_id": "Master document must belong to the group."}
                ) from exc
        else:
            master_document = max(
                documents,
        ...
        storage_saved_bytes = max(
            sum(int(document.size or 0) for document in documents)
            - int(master_document.size or 0),
            0,
        )
        record, _ = DuplicateDecision.objects.update_or_create(
            group_key=group_key,
            defaults={
                ...
                "document_class": next(
                    (document.category for document in documents if document.category),
                    "",
                ),
            },
        )
...
class DocumentOcrProcessingService:
    ...
    def _extract_page_payloads(
        document: Document, extracted_text: str, confidence: float | None, engine: str
    ) -> list[dict]:
    ...
    def _extract_entities(
        page_payloads: list[dict], confidence: float | None, engine: str
    ) -> list[dict]:
        entities: list[dict] = []
            ...
            for pattern_key, pattern in PATTERN_REGISTRY.items():
                ...
                for match in pattern.finditer(page_text):
                    ...
                    if not raw_value:
                        continue
                    normalized_value = (
                        DocumentOcrProcessingService._normalize_entity_value(
    ...
    def process_job(cls, job: OcrJob):
        ...
        page_payloads = cls._extract_page_payloads(
            document, result.text, result.confidence, result.engine
        )
        entity_payloads = cls._extract_entities(
            page_payloads, result.confidence, result.engine
        )
        ...
        DocumentOcrEntity.objects.bulk_create(
            [
                DocumentOcrEntity(document=document, **entity)
...
class OcrApplicationService:
    @staticmethod
    def start_job(document_id, user=None, request=None):
        if getattr(user, "is_superuser", False) or getattr(user, "is_staff", False):
            document = Document.objects.get(id=document_id)
        elif getattr(user, "is_authenticated", False):
            document = PermissionService.scope_queryset(
                Document.objects.all(), user, "view_document"
            ).get(id=document_id)
        else:
            document = Document.objects.get(id=document_id)
        ...
        ip_address = request.META.get("REMOTE_ADDR") if request else None
        AuditService.log(
            "OCR", "OCR", user=user, entity=document_id, ip_address=ip_address
        )
        EventService.publish(
            "OcrJobQueued",
            "OcrJob",
    ...
    def result_for_document(document_id, user=None):
        ...
        entities = list(
            DocumentOcrEntity.objects.filter(document=document)
