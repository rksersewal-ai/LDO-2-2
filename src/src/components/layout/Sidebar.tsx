...
import { 
  ...
  Settings, ServerCog, DatabaseBackup, Search, Menu,
            ...
            <AnimatePresence>
              {isExpanded && (
                <motion.div 
                  ...
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                  className="px-3 mb-2 text-[10px] font-semibold text-slate-500 uppercase tracking-widest whitespace-nowrap"
                >
                  {group.label}
                </motion.div>
              )}
                ...
                return (
                  ...
                  >
                    {isActive && (
                      <motion.div 
                        layoutId="activeNav" 
                        className="absolute left-0 top-0 bottom-0 w-1 bg-teal-400 rounded-r-full shadow-[0_0_10px_rgba(45,212,191,0.5)]" 
                      />
                    )}
                    <item.icon className={`w-5 h-5 shrink-0 ${isActive ? 'text-teal-400' : 'group-hover:text-slate-300'}`} />
                    <AnimatePresence>
                      {isExpanded && (
                        <motion.span
                          initial={{ opacity: 0, x: -10 }}
