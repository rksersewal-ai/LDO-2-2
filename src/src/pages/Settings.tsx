    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      <div>
        ...
        <p className="text-slate-400 text-sm">
          Application configuration and system defaults.
        </p>
      </div>
      ...
      <div className="flex gap-6">
        ...
        <div className="w-56 shrink-0 space-y-1">
          {settingGroups.map((g) => (
            ...
            >
              {g.label}
              <ChevronRight
                className={`w-4 h-4 ${activeGroup === g.label ? "text-teal-400" : "text-slate-600"}`}
              />
            </button>
          ))}
        </div>
        ...
        <GlassCard className="flex-1 p-6">
          <h2 className="text-lg text-white mb-6" style={{ fontWeight: 700 }}>
            {group.label}
          </h2>
            ...
            {group.settings.map((s) => (
              ...
              >
                ...
                {s.type === "toggle" && (
                  <button
                    onClick={() =>
                      setValues((v) => ({ ...v, [s.key]: !v[s.key] }))
                    }
                    className={`w-12 h-6 rounded-full transition-colors relative ${values[s.key] ? "bg-teal-500" : "bg-slate-700"}`}
                  >
                    <span
                      className={`absolute top-0.5 w-5 h-5 rounded-full bg-white shadow transition-transform ${values[s.key] ? "translate-x-6" : "translate-x-0.5"}`}
                    />
                  </button>
                )}
              </div>
