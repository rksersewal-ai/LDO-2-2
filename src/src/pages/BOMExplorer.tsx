...
function tagColor(tag: string) {
  ...
  if (t.includes("wear") || t.includes("consumable"))
    return "bg-orange-900/50 text-orange-300 border-orange-500/30";
  if (t.includes("structural") || t.includes("forged"))
...
function NodeIcon({
  ...
  if (type === "assembly")
    return <Box className={`${className} text-blue-400 shrink-0`} />;
  if (type === "sub-assembly")
    return <Layers className={`${className} text-indigo-400 shrink-0`} />;
...
const PRODUCT_BOMS: ProductBOM[] = [
  {
    ...
    code: "WAP-7",
    description:
      "25kV AC electric locomotive for mainline passenger service. 6120 HP, regenerative braking.",
    status: "Production",
    version: "D",
    totalParts: 35,
    assemblies: 10,
  ...
  },
  {
    ...
    code: "WAG-9HC",
    description:
      "High-capacity 25kV AC freight locomotive. 6350 HP, axle load 22.9t.",
    status: "Production",
    version: "C",
    totalParts: 42,
    assemblies: 12,
  ...
  },
  {
    ...
    code: "DETC",
    description:
      "Diesel-Electric Trailer Coach for DEMUs. AC traction, 110 km/h design speed.",
    status: "Production",
    version: "B",
    totalParts: 28,
    assemblies: 8,
  ...
  {
    id: "tm6fra",
    name: "Traction Motor 6FRA",
    code: "6FRA-6068",
    description:
      "3-phase AC traction motor, 1020 kW continuous. Class H insulation, forced ventilation.",
    status: "Production",
    version: "B",
    totalParts: 18,
    assemblies: 4,
  ...
  },
  {
    ...
    code: "PAN-HI",
    description:
      "High-speed single-arm pantograph for 25kV OHE current collection.",
    status: "Prototype",
    version: "A",
    totalParts: 8,
    assemblies: 2,
...
const BOM_VERSIONS_HISTORY = [
  {
    ...
    label: "Rev D — Serial 30601+ batch",
  },
  {
    version: "C",
    date: "2025-09-15",
    author: "A. Sharma",
    label: "Rev C — Traction motor insulation upgrade",
  },
  {
    version: "B",
    date: "2025-03-01",
    author: "D. Mukherjee",
    label: "Rev B — Bogie gusset modification",
  },
  {
    version: "A",
    date: "2024-08-01",
    author: "R. Krishnamurthy",
    label: "Rev A — Initial release",
  },
];

function ProductCardsLanding({
  onSelectProduct,
  canCreate,
}: {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-3xl text-white mb-1" style={{ fontWeight: 700 }}>
            BOM Explorer
          </h1>
          <p className="text-slate-400 text-sm">
            Select a product to explore its Bill of Materials structure.
          </p>
        </div>
        {canCreate && (
          <Button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {PRODUCT_BOMS.map((product) => (
          ...
          >
            ...
            <h3
              className="text-base text-white group-hover:text-teal-300 transition-colors mb-0.5"
...
function ContextMenu({
  ...
  return (
    <div
      ref={ref}
      className="fixed z-50 bg-slate-900/95 backdrop-blur-xl border border-teal-500/20 rounded-xl shadow-2xl shadow-black/50 py-1.5 min-w-[200px]"
      style={{
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">
              This will restructure the BOM hierarchy
            </p>
          </div>
        </div>
        <div className="space-y-3 mb-6">
          ...
          <div className="flex items-center gap-3">
            <div className="flex-1 p-3 rounded-lg bg-slate-800/50">
              ...
              <p className="text-xs text-rose-400">{move.fromParentName}</p>
            </div>
            ...
            <div className="flex-1 p-3 rounded-lg bg-teal-500/10 border border-teal-500/20">
              ...
              <p className="text-xs text-teal-300">{move.toParentName}</p>
            </div>
          </div>
        </div>
...
function TreeNode({
  ...
  onContextMenu: (e: React.MouseEvent, node: BOMNode) => void;
  onRequestMove: (nodeId: string, targetId: string) => void;
}) {
  ...
  const [{ isOver, canDrop }, drop] = useDrop({
    ...
    canDrop: (item: DragItem) =>
      node.type !== "part" && item.id !== node.id && item.parentId !== node.id,
    drop: (item: DragItem) => {
      onRequestMove(item.id, node.id);
    },
    collect: (monitor) => ({
  ...
  const isSafetyVital = node.tags.some((t) =>
  ...
  return (
    <div style={{ opacity: isDragging ? 0.4 : 1 }}>
      <div
        ...
        onClick={(e) => {
          if (e.shiftKey || e.ctrlKey || e.metaKey) onMultiSelect(node.id);
      ...
      >
        ...
        <code
          className={`text-[11px] shrink-0 font-mono ${nodeTypeColor(node.type)}`}
        >
          {node.id}
        </code>
        <span
          className="text-xs text-slate-200 truncate flex-1"
          style={{ fontWeight: hasChildren ? 500 : 400 }}
        >
          {node.name}
        </span>
        {node.quantity > 1 && (
          <span className="text-[10px] bg-teal-900/30 text-teal-400 px-1.5 py-0.5 rounded border border-teal-500/20 shrink-0">
            x{node.quantity}
          </span>
        )}
        <span className="text-[10px] text-slate-600 shrink-0 hidden xl:inline">
          Rev {node.revision}
        </span>
        {hasChildren && (
        ...
        <button
          ...
          className="opacity-0 group-hover:opacity-100 p-0.5 hover:bg-slate-700/50 rounded transition-all shrink-0"
      ...
      {hasChildren && isExpanded && (
        <div className="relative" role="group">
          <div
            className="absolute left-0 top-0 bottom-0 border-l border-slate-700/30"
            style={{ marginLeft: (depth + 1) * 20 + 12 }}
          />
      ...
      <div className="flex items-center justify-center h-full text-slate-600">
        <div className="text-center px-4">
          ...
          <p className="text-xs text-slate-700 mt-1">
            Click any item in the BOM tree
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="p-4 border-b border-slate-700/30">
        ...
        <h2 className="text-sm text-white mb-1" style={{ fontWeight: 600 }}>
          {node.name}
        </h2>
        <div className="flex items-center gap-2 flex-wrap">
          <code className="text-[10px] text-teal-400 font-mono bg-teal-500/10 px-2 py-0.5 rounded">
            PL {node.id}
          </code>
          ...
          {isSafety && (
            <Badge variant="danger" className="text-[8px] px-1.5">
              Safety Vital
            </Badge>
          )}
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          {[
            ...
            <div
              key={item.label}
              className="p-2 rounded-lg bg-slate-800/20 border border-slate-700/20"
            >
        ...
        <div className="flex flex-wrap gap-1">
          {node.tags.map((tag) => (
            <span
              key={tag}
              className={`text-[8px] px-1.5 py-0.5 rounded-full border ${tagColor(tag)}`}
            >
              {tag}
            </span>
          ))}
        ...
        {parent && (
          <button
            ...
            className="w-full flex items-center gap-2 p-2 rounded-lg bg-slate-800/20 border border-slate-700/20 text-left hover:border-teal-500/30 transition-colors"
          >
...
function compareTrees(oldTree: BOMNode[], newTree: BOMNode[]) {
  ...
  for (const [id, nn] of nM) {
    ...
    if (on.revision !== nn.revision)
      d.push({ field: "Rev", from: on.revision, to: nn.revision });
    if (on.quantity !== nn.quantity)
      d.push({
        field: "Qty",
        from: String(on.quantity),
        to: String(nn.quantity),
      });
    const op = findParent(oldTree, id),
      np = findParent(newTree, id);
    if ((op?.id || "") !== (np?.id || ""))
      d.push({
        field: "Parent",
        from: op?.name || "Root",
...
function BOMCompareView({
  ...
  return (
    <div
      ...
      onClick={onClose}
      role="dialog"
      aria-label="BOM Compare"
    >
      <GlassCard
        className="w-[900px] max-w-[95vw] h-[80vh] flex flex-col"
        onClick={(e: React.MouseEvent) => e.stopPropagation()}
      >
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-3 text-xs">
              <span className="flex items-center gap-1 text-emerald-400">
                <Plus className="w-3 h-3" /> {diff.added.size}
              </span>
              <span className="flex items-center gap-1 text-rose-400">
                <Minus className="w-3 h-3" /> {diff.removed.size}
              </span>
              <span className="flex items-center gap-1 text-amber-400">
                <RefreshCw className="w-3 h-3" /> {diff.changed.size}
              </span>
            </div>
            <button
              onClick={onClose}
        ...
        <div className="px-5 py-3 border-b border-slate-800 flex items-center gap-4">
          <div className="flex-1">
            <p className="text-[10px] text-slate-500 uppercase mb-1">Base</p>
            <select
              value={lv}
          ...
          diff.changed.size === 0 ? (
            <div className="flex items-center justify-center h-full">
              <div className="text-center">
                <Check className="w-10 h-10 mx-auto mb-3 text-teal-500" />
                <p className="text-sm text-teal-400">No differences</p>
              </div>
            </div>
          ) : (
            allIds.map((id) => {
              const ia = diff.added.has(id),
                ir = diff.removed.has(id),
                ch = diff.changed.get(id);
              if (!ia && !ir && !ch) return null;
              const n = findNode(rt, id) || findNode(lt, id);
              if (!n) return null;
              return (
                <div
                  key={id}
                  className={`px-3 py-2 rounded-lg border text-xs ${ia ? "bg-emerald-500/10 border-emerald-500/20" : ir ? "bg-rose-500/10 border-rose-500/20" : "bg-amber-500/5 border-amber-500/15"}`}
                >
                  <div className="flex items-center gap-2">
                    {ia && <Plus className="w-3.5 h-3.5 text-emerald-400" />}
                    {ir && <Minus className="w-3.5 h-3.5 text-rose-400" />}
                    {ch && <RefreshCw className="w-3.5 h-3.5 text-amber-400" />}
                    <NodeIcon type={n.type} className="w-3.5 h-3.5" />
                    <code className="font-mono text-slate-400">{id}</code>
                    <span
                      className={
                        ia
                          ? "text-emerald-300"
                    ...
                    {ch && (
                      <Badge variant="warning" className="text-[8px] px-1.5">
                        MODIFIED
                      </Badge>
                    )}
                  </div>
...
function EffectivityFilter({
  onClose,
  onApply,
}: {
  onClose: () => void;
  onApply: (f: { dateFrom?: string; dateTo?: string; serial?: string }) => void;
}) {
      ...
      <table className="w-full text-left text-xs">
        <thead>
          <tr className="border-b border-slate-700/50 text-[10px] text-slate-500 uppercase tracking-wider">
            <th className="py-2 px-3">PL Number</th>
            <th className="py-2 px-3">Name</th>
            <th className="py-2 px-3">Rev</th>
            <th className="py-2 px-3">Qty</th>
            <th className="py-2 px-3">Parent Path</th>
        ...
        <tbody className="divide-y divide-slate-800/40">
          {partsOnly.map((item) => (
            <tr
              key={`${item.plNumber}-${item.parentPath}`}
              onClick={() => onNavigate(item.plNumber)}
            ...
            >
              ...
              <td className="py-2 px-3 text-slate-600 text-[10px] max-w-[200px] truncate">
                {item.parentPath}
              </td>
              <td className="py-2 px-3">
                <div className="flex gap-1">
                  {item.tags.slice(0, 2).map((tag) => (
                    <span
                      key={tag}
                      className={`text-[8px] px-1.5 py-0 rounded-full border ${tagColor(tag)}`}
                    >
                      {tag}
                    </span>
                  ))}
...
export default function BOMExplorer() {
  ...
  const [history, setHistory] = useState<BOMNode[][]>([
    cloneTree(INITIAL_BOM_TREE),
  ]);
  ...
  const [versions, setVersions] = useState<BOMVersion[]>([
    {
      ...
      tree: cloneTree(INITIAL_BOM_TREE),
    },
  ]);
  ...
  const [effectivityFilter, setEffectivityFilter] = useState<{
    dateFrom?: string;
    dateTo?: string;
    serial?: string;
  }>({});
  const hasEffectivity = !!(
    effectivityFilter.dateFrom ||
    effectivityFilter.dateTo ||
    effectivityFilter.serial
  );
  const [showVersionHistory, setShowVersionHistory] = useState(false);

  useEffect(() => {
    if (searchMatches.size > 0)
      setExpanded((prev) => new Set([...prev, ...searchMatches]));
  }, [searchMatches]);

  const focusedNodeData = useMemo(
    () => (focusedNode ? findNode(tree, focusedNode) : null),
    [tree, focusedNode],
  );
  const focusedPLRecord = useMemo(
    () => (focusedNode ? getPLRecord(focusedNode) : undefined),
    [focusedNode],
  );
  ...
  const handleContextMenu = useCallback(
    (e: React.MouseEvent, node: BOMNode) => {
      e.preventDefault();
      e.stopPropagation();
  ...
  const handleContextAction = useCallback(
    (action: string, nodeId: string) => {
      if (action === "remove" && canEdit) {
        ...
        const rem = (arr: BOMNode[]): boolean => {
          for (let i = 0; i < arr.length; i++) {
            if (arr[i].id === nodeId) {
              arr.splice(i, 1);
              return true;
            }
            if (rem(arr[i].children)) return true;
          }
          return false;
        };
        rem(t);
  ...
  const handleRequestMove = useCallback(
    (nodeId: string, targetId: string) => {
      if (!canEdit) return;
      const s = findNode(tree, nodeId);
      const sp = findParent(tree, nodeId);
      const t = findNode(tree, targetId);
      if (!s || !t) return;
      setPendingMove({
        nodeId,
        nodeName: s.name,
        fromParentId: sp?.id || null,
        fromParentName: sp?.name || "Root",
        toParentId: targetId,
        toParentName: t.name,
      });
    },
  ...
  const confirmMove = useCallback(() => {
    ...
    if (newTree) {
      pushHistory(newTree);
      setExpanded((prev) => new Set([...prev, pendingMove.toParentId]));
    }
    setPendingMove(null);
    ...
    (id: string) => {
      ...
      const findPath = (
        ...
        for (const n of nodes) {
          ...
          if (findPath(n.children, target, [...current, n.id])) return true;
        }
        return false;
      };
      findPath(tree, id, []);
  ...
  return (
    <DndProvider backend={HTML5Backend}>
      <div className="h-[calc(100vh-8rem)] flex flex-col gap-3">
          ...
          <div>
            <div className="flex items-center gap-3 mb-1">
              ...
              <h1 className="text-2xl text-white" style={{ fontWeight: 700 }}>
                {PRODUCT_BOMS.find((p) => p.id === selectedProduct)?.name ||
                  "BOM Explorer"}
              </h1>
            ...
            <p className="text-slate-400 text-xs ml-10">
              {PRODUCT_BOMS.find((p) => p.id === selectedProduct)?.code} —{" "}
              {stats.assemblies} assemblies, {stats.parts} parts
            </p>
          </div>
          <div className="flex items-center gap-2">
            <Button
        ...
        {showVersionHistory && (
          <GlassCard className="p-4">
            <h3 className="text-xs text-white mb-3" style={{ fontWeight: 600 }}>
              BOM Version History
            </h3>
            <div className="space-y-2">
              {BOM_VERSIONS_HISTORY.map((v, i) => (
                <div
                  ...
                  onClick={() => {
                    setTree(cloneTree(INITIAL_BOM_TREE));
                    setHasUnsavedChanges(false);
                ...
                >
                  <div
                    className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs ${i === 0 ? "bg-teal-500/20 text-teal-300" : "bg-slate-800 text-slate-500"}`}
                    style={{ fontWeight: 600 }}
                  >
                    {v.version}
                  </div>
        ...
        <GlassCard className="px-4 py-2.5 flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <div className="relative">
              ...
              <Input
                ...
                placeholder="Search PL number, name, or tag..."
                className="pl-9 w-64"
            ...
            <Button
              variant="ghost"
              onClick={() => setShowEffectivity(true)}
              className={`px-2 py-1.5 ${hasEffectivity ? "text-amber-400" : ""}`}
              aria-label="Effectivity filter"
            >
              <Filter className="w-4 h-4" />
              <span className="hidden lg:inline text-xs">
                {hasEffectivity ? "Effectivity (Active)" : "Effectivity"}
              </span>
            </Button>
            <Button
              variant="ghost"
          ...
          <Button
            ...
            aria-label={showDetail ? "Hide detail" : "Show detail"}
          >
            {showDetail ? (
        ...
        <div className="flex-1 flex gap-4 min-h-0">
          <GlassCard
            className={`flex flex-col min-h-0 ${showDetail ? "flex-1" : "w-full"}`}
          >
            <div className="px-4 py-3 border-b border-slate-700/30 flex items-center justify-between bg-slate-900/30">
              ...
              <div className="flex items-center gap-3 text-[10px] text-slate-600">
                <span className="flex items-center gap-1">
                  <Box className="w-3 h-3 text-blue-400" /> Assembly
                </span>
                <span className="flex items-center gap-1">
                  <Layers className="w-3 h-3 text-indigo-400" /> Sub-Assy
                </span>
                <span className="flex items-center gap-1">
                  <Cpu className="w-3 h-3 text-slate-400" /> Part
                </span>
                <span className="flex items-center gap-1">
                  <Shield className="w-3 h-3 text-rose-400" /> Safety
                </span>
              </div>
            </div>
          ...
          <div className="flex items-center gap-2">
            {canEdit ? (
              ...
              <span className="text-xs text-slate-500 flex items-center gap-1">
                <Lock className="w-3 h-3" /> View-only mode — contact
                admin/supervisor to edit
              </span>
            )}
          </div>
      ...
      {showEffectivity && (
        <EffectivityFilter
          onClose={() => setShowEffectivity(false)}
          onApply={(f) => {
            setEffectivityFilter(f);
            setShowEffectivity(false);
          }}
        />
      )}
    </DndProvider>
