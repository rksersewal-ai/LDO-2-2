...
const byStatus = [
  {
    status: "Complete",
    count: MOCK_WORK_LEDGER.filter((w) => w.status === "Complete").length,
  },
  {
    status: "In Progress",
    count: MOCK_WORK_LEDGER.filter((w) => w.status === "In Progress").length,
  },
  {
    status: "Pending",
    count: MOCK_WORK_LEDGER.filter((w) => w.status === "Pending").length,
  },
  {
    status: "Verification",
    count: MOCK_WORK_LEDGER.filter((w) => w.status === "Pending Verification")
      .length,
  },
];
...
export default function LedgerReports() {
  ...
  const completed = MOCK_WORK_LEDGER.filter(
    (w) => w.status === "Complete",
  ).length;
    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-slate-400 text-sm">
            Summary views and operational reporting for work records.
          </p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <GlassCard className="p-5">
          ...
          <div className="text-3xl text-teal-400" style={{ fontWeight: 700 }}>
            {completed}
          </div>
        </GlassCard>
        <GlassCard className="p-5">
          <div className="text-xs text-slate-400 mb-1">Open</div>
          <div className="text-3xl text-amber-400" style={{ fontWeight: 700 }}>
            {total - completed}
          </div>
        </GlassCard>
        <GlassCard className="p-5">
        ...
        <table className="w-full text-left text-sm whitespace-nowrap">
          ...
          <tbody className="divide-y divide-slate-800/50 text-slate-300">
            {MOCK_WORK_LEDGER.map((w) => (
              ...
              >
                <td className="py-3 pl-4 font-mono text-teal-400 text-xs">
                  {w.id}
                </td>
