    ...
    <div className="flex items-center gap-3 py-2">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          <code className="text-[10px] bg-slate-800 text-teal-400 px-1.5 py-0.5 rounded">
            {tailwind}
          </code>
        </div>
  ...
  return (
    <div className="mb-4">
      <button
        ...
        className="flex items-center gap-2 w-full text-left py-2 px-1 hover:bg-slate-800/30 rounded-lg transition-colors"
      >
        {open ? (
        ...
        <span className="text-sm text-slate-200" style={{ fontWeight: 600 }}>
          {title}
        </span>
      </button>
      ...
      >
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            <code className="text-[10px] bg-teal-900/40 text-teal-400 px-1.5 py-0.5 rounded">
              {comp.id}
            </code>
        ...
        {expanded ? (
          <ChevronDown className="w-4 h-4 text-teal-400 shrink-0 mt-1" />
        ) : (
      ...
      {expanded && (
        <div className="mt-4 space-y-4 border-t border-slate-800 pt-4">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1">
              {comp.usedIn.map((s, i) => (
                <span
                  key={i}
                  className="text-[10px] bg-teal-900/30 text-teal-400 px-2 py-0.5 rounded-full"
                >
                  {s}
                </span>
              ))}
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 font-mono bg-slate-950/50 p-3 rounded-lg">
              {comp.handoffNotes}
            </p>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div>
              <h4 className="text-xs text-teal-500 uppercase tracking-wider mb-2 flex items-center gap-1">
            ...
            <div>
              <h4 className="text-xs text-rose-500 uppercase tracking-wider mb-2 flex items-center gap-1">
                <XCircle className="w-3 h-3" /> Do Not
              </h4>
              <div className="space-y-1">
          ...
          <div className="bg-slate-800/30 rounded-lg p-3">
            <h4 className="text-xs text-amber-500 uppercase tracking-wider mb-1">
              Future Considerations
            </h4>
            <p className="text-xs text-slate-500">
              {comp.futureConsiderations}
            </p>
          </div>
        </div>
      )}
    ...
    <div className="space-y-2">
      ...
      <Section title="Typography Scale">
        <div className="space-y-2">
          {typography.scale.map((t) => (
            <div
              key={t.name}
              className="flex items-baseline gap-4 py-1.5 border-b border-slate-800/50"
            >
              <code className="text-[10px] bg-slate-800 text-teal-400 px-1.5 py-0.5 rounded w-24 text-center shrink-0">
                {t.name}
              </code>
              <span className="text-slate-300" style={{ fontSize: t.size }}>
                {t.size}
              </span>
        ...
        <div className="space-y-1">
          {Object.entries(typography.fontWeights).map(([name, fw]) => (
            <div key={name} className="flex items-center gap-3 py-1">
              <span
                className="text-sm text-slate-200 w-24"
      ...
      <Section title="Text Size Control">
        <div className="bg-slate-800/30 rounded-lg p-4 text-xs text-slate-400 space-y-1">
          <p>
            Range: {typography.textSizeControl.min}px -{" "}
            {typography.textSizeControl.max}px (default:{" "}
            {typography.textSizeControl.default}px)
          </p>
          <p>
            Step: {typography.textSizeControl.stepSize}px | CSS Variable:{" "}
            <code className="text-teal-400">
              {typography.textSizeControl.cssVariable}
            </code>
          </p>
          <p className="text-slate-500 mt-2">
            {typography.textSizeControl.rationale}
          </p>
        </div>
      </Section>
        ...
        <div className="space-y-1">
          {spacing.scale.map((s) => (
            <div key={s.token} className="flex items-center gap-3 py-1">
              <code className="text-[10px] bg-slate-800 text-teal-400 px-1.5 py-0.5 rounded w-20 text-center shrink-0">
                {s.token}
              </code>
              <div
                className="bg-teal-500/30 h-3 rounded"
                style={{ width: s.value }}
              />
        ...
        <div className="flex flex-wrap gap-4">
          {Object.entries(radii).map(([name, r]) => (
            <div key={name} className="text-center">
              <div
                className="w-16 h-16 bg-teal-500/20 border border-teal-500/30 mx-auto mb-2"
        ...
        <div className="space-y-2">
          {Object.entries(elevation).map(([name, val]) => (
            <div key={name} className="flex items-center gap-3 py-1">
              ...
              <code className="text-[10px] bg-slate-800 text-teal-400 px-2 py-0.5 rounded">
                {val}
              </code>
            </div>
          ))}
      ...
      <Section title="Accessibility">
        <div className="space-y-3">
          <div>
            ...
            <code className="text-[10px] bg-slate-800 text-teal-400 px-2 py-1 rounded">
          ...
          <div>
            ...
            {Object.entries(accessibility.contrastRatios).map(
              ([name, ratio]) => (
                <div
                  key={name}
                  className="flex gap-2 text-xs text-slate-500 py-0.5"
                >
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
      <div className="lg:col-span-1 space-y-2">
        <h3 className="text-sm text-slate-300 mb-3" style={{ fontWeight: 600 }}>
          All Screens ({SCREEN_MAP.length})
        </h3>
        {SCREEN_MAP.map((scr) => (
          <button
            key={scr.id}
            onClick={() => setSelected(scr.id)}
        ...
        {selected ? (
          (() => {
            ...
            return (
              <GlassCard className="p-6 sticky top-0">
                <div className="flex items-center gap-2 mb-1">
                  <code className="text-xs bg-teal-900/40 text-teal-400 px-2 py-0.5 rounded">
                    {scr.id}
                  </code>
                  <h2
                ...
                <code className="text-xs text-slate-500 block mb-4">
                  {scr.path} - {scr.file}
                </code>
                ...
                <div className="space-y-5">
                  ...
                  <div>
                    <h4 className="text-xs text-slate-500 uppercase tracking-wider mb-2">
                      Components Used ({scr.components.length})
                    </h4>
                    <div className="flex flex-wrap gap-1">
                      {scr.components.map((cid) => {
                        const c = COMPONENT_INVENTORY.find(
                          (cc) => cc.id === cid,
                        );
                        return (
                          <span
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {scr.flows.map((fid) => {
                          ...
                          return (
                            <Badge key={fid} variant="processing">
                              {fid}: {f?.name || "Unknown"}
                            </Badge>
                          );
                        })}
          ...
          <div className="flex items-center justify-center h-64 text-slate-600 text-sm">
            <div className="text-center">
              ...
              <p>
                Select a screen to view its component map and UX documentation
              </p>
            </div>
          </div>
        )}
              ...
              <div>
                ...
                <div className="space-y-2 relative">
                  ...
                  {flow.steps.map((step, i) => (
                    <div
                      key={i}
                      className="flex items-start gap-3 text-xs text-slate-400 relative"
                    >
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    {flow.screens.map((sid) => {
                      ...
                      return (
                        <div key={sid} className="text-xs text-slate-400">
                          {sid}: {s?.name}
                        </div>
                      );
                    })}
      ...
      <table className="w-full text-left">
        <thead>
          <tr className="border-b border-slate-800">
            <th className="text-xs text-slate-500 uppercase tracking-wider py-3 px-4">
              State
            </th>
            <th className="text-xs text-slate-500 uppercase tracking-wider py-3 px-4">
              Description
        ...
        <tbody>
          {STATE_MATRIX.map((state, i) => (
            ...
            >
              ...
              <td className="py-3 px-4">
                <code className="text-[10px] text-teal-400 bg-slate-800 px-2 py-0.5 rounded">
                  {state.visualTreatment}
                </code>
              </td>
              <td className="py-3 px-4">
                <div className="flex flex-wrap gap-1">
                  {state.applicableTo.map((a) => (
                    <span
                      key={a}
                      className="text-[10px] bg-slate-800/50 text-slate-500 px-1.5 py-0.5 rounded"
                    >
                      {a}
                    </span>
                  ))}
...
function MatrixTab() {
  return (
    <div>
      <p className="text-xs text-slate-500 mb-4">
        Full Component Inventory Matrix - maps every component to its variants,
        states, screens, and handoff notes.
      </p>
      <div className="overflow-x-auto">
        <table className="w-full text-left min-w-[1000px]">
          <thead>
            <tr className="border-b border-slate-800">
              <th className="text-[10px] text-slate-500 uppercase tracking-wider py-3 px-3 w-16">
                ID
              </th>
              <th className="text-[10px] text-slate-500 uppercase tracking-wider py-3 px-3 w-32">
                Component
              </th>
              <th className="text-[10px] text-slate-500 uppercase tracking-wider py-3 px-3 w-24">
                Category
              </th>
              <th className="text-[10px] text-slate-500 uppercase tracking-wider py-3 px-3">
                Variants
              </th>
              <th className="text-[10px] text-slate-500 uppercase tracking-wider py-3 px-3">
                States
          ...
          <tbody>
            {COMPONENT_INVENTORY.map((comp, i) => (
              <tr
                key={comp.id}
                className={`border-b border-slate-800/30 ${i % 2 === 0 ? "bg-slate-900/20" : ""} hover:bg-slate-800/30`}
              >
                <td className="py-2 px-3">
                  <code className="text-[10px] text-teal-400">{comp.id}</code>
                </td>
                <td
                  className="py-2 px-3 text-xs text-slate-200"
                ...
                <td className="py-2 px-3">
                  <div className="flex flex-wrap gap-0.5">
                    {comp.states.map((s, si) => (
                      <span
                        key={si}
                        className="text-[9px] bg-slate-800 text-slate-500 px-1 py-0.5 rounded"
                      >
                        {s}
                      </span>
                    ))}
                ...
                <td className="py-2 px-3 max-w-[200px]">
                  <div className="text-[10px] text-slate-600 truncate">
                    {comp.handoffNotes.substring(0, 80)}...
                  </div>
                </td>
              </tr>
            ))}
        ...
        <GlassCard key={fi} className="p-4">
          ...
          <div className="space-y-1.5">
            {file.pages.map((page, pi) => (
              <div
                key={pi}
                className="flex items-start gap-3 py-1.5 px-3 rounded-lg hover:bg-slate-800/30 transition-colors"
              >
    ...
    <div className="space-y-6 max-w-[1400px] mx-auto">
      <div className="flex items-end justify-between mb-2">
        <div>
          ...
          <p className="text-slate-400 text-sm">
            LDO-2 EDMS - Living design documentation, component inventory, and
            UX reference
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <GlassCard className="p-4 text-center">
          <div
            className="text-2xl text-teal-400 mb-1"
        ...
        <GlassCard className="p-4 text-center">
          <div
            className="text-2xl text-emerald-400 mb-1"
            style={{ fontWeight: 700 }}
          >
            {totalScreens}
          </div>
